    ...
    try {
      ...
      block = block.replace(/\[(code|quote|table|material|plot)\]\n?([\s\S]*?)\n?\[\/(?:code|quote|table|material|plot)\]/gs, (match, type, content) => {
        ...
        if (type === 'code') {
          ...
          materialsHtml += `<div class="material-box"><div id="${plotId}" class="function-plot-container"></div><script>(function(){try{const config=${JSON.stringify(plotConfig)};config.target='#${plotId}';function createLegend(container,data){if(!container)return;const legend=document.createElement('div');legend.setAttribute('style','margin-top:10px;padding:10px;border:1px solid #ccc;border-radius:5px;font-family:sans-serif;font-size:14px;');data.forEach(d=>{const item=document.createElement('div');item.setAttribute('style','display:flex;align-items:center;margin-bottom:5px;');const swatch=document.createElement('span');swatch.setAttribute('style','display:inline-block;width:12px;height:12px;margin-right:8px;border:1px solid #777;background-color:'+d.color+';');const label=document.createElement('span');label.style.fontFamily='monospace';label.textContent=d.fn;item.appendChild(swatch);item.appendChild(label);legend.appendChild(item);});if(legend.lastChild){legend.lastChild.style.marginBottom='0';}container.appendChild(legend);}const plotInstance=functionPlot(config);createLegend(document.getElementById('${plotId}'),config.data);const originalXDomain=[...config.xAxis.domain];const originalYDomain=[...config.yAxis.domain];document.getElementById('${plotId}').addEventListener('dblclick',function(e){e.preventDefault();const container=document.getElementById('${plotId}');if(container){try{while(container.firstChild){container.removeChild(container.firstChild)}const resetConfig=JSON.parse(JSON.stringify(config));resetConfig.target='#${plotId}';resetConfig.xAxis.domain=[...originalXDomain];resetConfig.yAxis.domain=[...originalYDomain];functionPlot(resetConfig);createLegend(container,resetConfig.data)}catch(err){console.error("Reset failed:",err)}}})}catch(e){console.error("Plot config error:",e);document.getElementById('${plotId}').innerHTML='<p class="error">Invalid plot configuration.</p>'}})();<\/script></div>`;
        }
        return '';
      });
